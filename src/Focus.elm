effect module Focus where { subscription = MySub } exposing (..)
...
onEffects router newSubs oldState =
  let
    spawn task =
      task
        |> Task.andThen (\state -> Process.spawn (Dom.onDocument "focusin" targetId (Platform.sendToSelf router << (,) True))
        |> Task.andThen (\enterPid -> Process.spawn (Dom.onDocument "focusout" targetId (Platform.sendToSelf router << (,) False))
...
onSelfMsg router (focus, id) state =
  case Dict.get id state.watchers of
    ...
    Just taggers ->
      let
        send tagger =
          Platform.sendToApp router (tagger focus)
      in
